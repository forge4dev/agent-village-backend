...
## Backend Prototype Submission

This repository now includes a dependency-free Node backend prototype for the trust-boundary challenge.
...
The server listens on `http://localhost:8787` and persists local prototype state in `data/living-home.json`.
...
The demo starts an isolated temporary HTTP server, then shows:

- an owner conversation that stores a private memory,
- an owner recall that can use that private memory,
- a stranger conversation that cannot access or leak that memory,
- a proactive scheduler tick that writes public-safe diary entries,
- a public agent-to-agent interaction,
- the public feed after the proactive behavior.
...
The tests cover the core trust boundary: owner memory storage/recall, stranger non-disclosure, and proactive diary generation that reacts to private memory without copying private facts into the public feed.

### Architecture

See `docs/ARCHITECTURE.md` for the one-page design writeup covering components, trust boundaries, scheduling, scaling, and observability.
